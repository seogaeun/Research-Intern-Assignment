...
#define MAX_WORKERS 8 // 최대 워커 수. CPU 코어수에 따라 필요시 수정하면 된다.
#define SERVER_PORT 5000 // 서버쪽 Base port번호. 서버도 멀티스레드이므로, Port번호가 여러개임.
...
char txt[100] = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
...
void* tx_t(void* arg) {
    ...
    while (1) {
        ...
        SendBuffer.opRW = op1;
        SendBuffer.key = key1;
        SendBuffer.value = value1;
        for (int i = 0; i < 100; i++)
            SendBuffer.pSize[i] = txt[i];
        printf("op: %d, key: %d, value: %d, id: %d\n", SendBuffer.opRW, SendBuffer.key, SendBuffer.value, SendBuffer.id);
...
void* rx_t(void* arg) {
    ...
    while (1) {
        ...
        if (n > 0) {
            ...
            if (Rid == RecvBuffer.id);
            else {
                if (RecvBuffer.opRW == 0)
                    printf("%d번째 패킷 도착! READ --- %d의 value값: %d\n", z, RecvBuffer.key, RecvBuffer.value);
                if (RecvBuffer.opRW == 1)
                    printf("%d번째 패킷 도착! WRITE --- 처리 완료\n", z);
    ...
    while (1) {
        ...
        if (readline(0, data_to_send, maxlen) > 0) {
            ...
            for (int j = 0; j < maxlen; j++) {
                requests[i][j] = data_to_send[j];
            }
            printf("명령%d : %s\n", i, requests[i]);
            //sprintf("%s\0", requests[i]);
        }
        if (i >= NUM_REQUESTS)
            break;
    }
